...
// now that we have our parsed queue, we can release the node we're holding up and only re-allocate one for running `sh` commands (like curling GitHub)
...
for (buildObj in queue) {
	...
	withEnv([
		...
		stage(identifier) {
			...
			if (env.BASHBREW_ARCH == 'gha') { // TODO might be worth moving this conditional up and only allocating a single "node" for *all* invocations of curl (instead of the really flappy behavior this will have, but that was the most straightforward to write in porting the script "real quick")
				node {
					withCredentials([
						string(
							variable: 'GH_TOKEN',
